...
class WebvidDataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_root, train_eval_split):
        """
        ...
        merged_df = merged_df.loc[merged_df["train_eval_split"] == train_eval_split]
    ...
    def _get_video_path(self, sample):
        rel_video_fp = str(sample['videoid'])
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            if 'caption' in sample_dict.keys():
                text = sample_dict['caption'].strip().replace("\n", " ")
            else:
