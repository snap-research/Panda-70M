...
from utils.data_representation.gather_scatter.gather_scatter_utils import gather_tensor, gather_list, gather_element, gather_temporal_element, scatter_temporal_tensor

from typing import List, Tuple, Dict, Union, Set

class TemporalDataEntryGatherScatter(DataEntryGatherScatter):
    """
    A data entries gather scatter that operates on the temporal dimension when available and operates only of the batch dimension if the temporal one is unavailable
    """
    ...
    def gather(self, src_data_entry: DataEntry, indexes: List[Tuple[int, List[int]]]) -> DataEntry:
        ...
        new_data = gather_temporal_element(data, indexes)
        new_mask = gather_temporal_element(mask, indexes)
    ...
    def scatter(self, src_data_entry: DataEntry, indexes: List[Tuple[int, List[int]]], dst_data_entry: DataEntry) -> DataEntry:
        ...
        # Scatters the tensor
        new_data = scatter_temporal_tensor(src_data_entry.data, indexes, data)
        new_data_entry.data = new_data
