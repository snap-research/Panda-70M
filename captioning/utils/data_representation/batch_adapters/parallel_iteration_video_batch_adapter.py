...
class ParallelIterationVideoBatchAdapter(BatchAdapter):
    """
    ...
    def batch_to_data_entries(self, batch: Any) -> DataEntries:
        ...
        """
        image_data = batch.data['video']

        # (batch_size, frames_count, features, height, width)
        ...
        # Image = 0, Video = 1
        video_image_labels = torch.ones(videos.shape[0], dtype=torch.long, device=videos.device)
